...
#define VIDEO_VISIBLE_COLS	(pGD->winSizeY)
#define VIDEO_VISIBLE_ROWS	(pGD->winSizeX)
...
int video_logo_width = 0;
int video_logo_height = 0;
#define AT_END	-1
#define AT_MID	-2

#ifdef	CONFIG_CONSOLE_CURSOR //using now
#ifdef	CURSOR_ON
...
#define VIDEO_LINE_LEN		(VIDEO_ROWS*VIDEO_PIXEL_SIZE)
#endif
...
#define CONSOLE_COL_SIZE	(VIDEO_FONT_WIDTH * VIDEO_COLS)
#define CONSOLE_COL_LAST	(video_console_address + (CONSOLE_SIZE - VIDEO_FONT_HEIGHT *2))
#define CONSOLE_COL_SIZE    (VIDEO_FONT_HEIGHT * VIDEO_PIXEL_SIZE * VIDEO_COLS)
#define CONSOLE_COL_FIRST   (video_console_address + (CONSOLE_SIZE - VIDEO_LINE_LEN))
...
static const unsigned short video_font_draw_table88[] = {
	    0x0000, 0xffff };
	...
	case GDF_16BIT_565RGB:
		while (count--) {
			...
			for (rows = VIDEO_FONT_HEIGHT, dest = dest0;
			     ...
			     dest += 2) {
				unsigned char bits = *cdat++;
				((unsigned short *) dest)[0] ^= video_font_draw_table88 [bits >> 7] ;
				((unsigned short *) (dest - VIDEO_LINE_LEN))[0] ^= video_font_draw_table88 [bits >> 6 & 1] ;
				((unsigned short *) (dest - VIDEO_LINE_LEN*2))[0] ^= video_font_draw_table88 [bits >> 5 & 1] ;
				((unsigned short *) (dest - VIDEO_LINE_LEN*3))[0] ^= video_font_draw_table88 [bits >> 4 & 1] ;
				((unsigned short *) (dest - VIDEO_LINE_LEN*4))[0] ^= video_font_draw_table88 [bits >> 3 & 1] ;
				((unsigned short *) (dest - VIDEO_LINE_LEN*5))[0] ^= video_font_draw_table88 [bits >> 2 & 1] ;
				((unsigned short *) (dest - VIDEO_LINE_LEN*6))[0] ^= video_font_draw_table88 [bits >> 1 & 1] ;
				((unsigned short *) (dest - VIDEO_LINE_LEN*7))[0] ^= video_font_draw_table88 [bits & 1] ;
#if 0
			...
			dest0 += VIDEO_FONT_WIDTH * VIDEO_ROWS * VIDEO_PIXEL_SIZE;
			s++;
		}
		break;
...
{
	unsigned char *cdat, *ccdat, *dest, *dest0;
	int rows, offset, c;
	int i;
...
#endif
#if 0
				((unsigned int *) dest)[0] = ((video_font_draw_table16 [bits >> 6] & eorx) ^ bgx);
				((unsigned int *) dest)[1] = ((video_font_draw_table16 [bits >> 4 & 3] & eorx) ^ bgx);
				((unsigned int *) dest)[2] = ((video_font_draw_table16 [bits >> 2 & 3] & eorx) ^ bgx);
				((unsigned int *) dest)[3] = ((video_font_draw_table16 [bits & 3] & eorx) ^ bgx);
#endif
...
#ifndef VIDEO_HW_BITBLT	//using now
...
//	video_drawchars (xx, yy + video_logo_height, &c, 1);
	video_drawchars (yy, VIDEO_VISIBLE_COLS-xx + video_logo_height, &c, 1);
#endif
}
...
{
#ifndef VIDEO_NO_CONSOLE
	video_drawchars_xor (yy, VIDEO_VISIBLE_COLS-xx + video_logo_height, &c, 1);
#endif
}
/*****************************************************************************/
#ifdef CONFIG_CONSOLE_CURSOR	//using now
...
static void console_scrollup (void)
{
   ...
   int i;
   short *len = CONSOLE_COL_FIRST;
   short tmp = VIDEO_COLS;
   while(tmp--){
       for(i=0;i<VIDEO_FONT_HEIGHT;i++)
           *(len+i) = CONSOLE_BG_COL;
       len = len - VIDEO_ROWS;
   }

#if defined(MEM_PRINTTO_VIDEO) //using now
	if (CONSOLE_ROWS > 1)
...
#ifdef CONFIG_VIDEO_HW_CURSOR	//not using
