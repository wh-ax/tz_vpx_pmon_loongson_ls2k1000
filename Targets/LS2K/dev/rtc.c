...
{
	ls2k_i2c1_writeb(TXR_REG, data);          //将date写入i2c1的数据发送寄存器 TX代表发送
	ls2k_i2c1_writeb(CR_REG, opt);            //将opt(start wirte信号)写入命令控制寄存器
	while (ls2k_i2c1_readb(SR_REG) & SR_TIP) ; //等待这个数据写入完成

	if (ls2k_i2c1_readb(SR_REG) & SR_NOACK) {   //如果没有收到 ack 应答
		printf("rtc has no ack, Pls check the hardware!\n");
		ls2k_i2c1_stop();                       //停止
		return -1;
	}
...
static int rtc_send_addr(u8 data_addr)
{
    ...
    if (i2c1_tx_byte(dev_addr, CR_START | CR_WRITE) < 0) //CR_STAR 
        return 0;
    if (i2c1_tx_byte(data_addr, CR_WRITE) < 0)
        return 0;
...
static int rtc_read_cur(u8 *buf, int count)
{
    ...
    for (i = 0; i < count; i++) {
        ls2k_i2c1_writeb(CR_REG, (    (i == count - 1) ? (CR_READ | CR_ACK) : CR_READ      ) ); //在i不等于1时一直写入读信号
        while (ls2k_i2c1_readb(SR_REG) & SR_TIP) ;
        buf[i] = ls2k_i2c1_readb(RXR_REG);
    }
...
int rtc_read_rand(u8 data_addr, u8 *buf,int count) //当rtc_get_time时 count=7
{
    //printf("-----i2c1_init------\n");
   // i2c1_init();
    //printf("rtc_read_rand\n");
	if (!rtc_send_addr(data_addr))
		return 0;

	if (!rtc_read_cur(buf,count))  //获取rtc真实的时间
		return 0;
	return 1;
}
...
int rtc_get_time(u8 *buf)
{
    ...
    //wjz 2 is the start register
    //printf("0x2 is the start register");
	if (rtc_read_rand(0x2, buf,7)) {
		/*Becuse the RTC API need ture value,we must calculate the ture number.*/
...
#endif	
	delay(1000);
	//0x2 is the start register.
	if (rtc_write(0x2, buf, 7))
		return 1;
	else
		return 0;
}
